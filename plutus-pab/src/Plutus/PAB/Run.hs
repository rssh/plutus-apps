...
import Cardano.Node.Types (PABServerConfig (pscSocketPath), pscPassphrase)
...
import Plutus.PAB.Types (Config (Config), DevelopmentOptions (pabResumeFrom, pabRollbackHistory),
                         PABError (FileNotFound, MissingConfigFileOption), developmentOptions, nodeServerConfig)
...
runWithOpts userContractHandler mc AppOpts { minLogLevel, rollbackHistory, resumeFrom, logConfigPath, passphrase, runEkgServer, cmd, configPath, storageBackend } = do
    ...
    where
        run config = do
            let socketPath = pscSocketPath $ nodeServerConfig $ ccaPABConfig config
            doesFileExist socketPath
                >>= \case True  -> pure ()
                          False -> handleError (FileNotFound socketPath)
            runConfigCommand userContractHandler config cmd
